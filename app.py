...
def download_image():
    ...
    print(filename)
    downloader.download(filename + " Spotify MP3 Album", limit=1, output_dir='',
...
# song_list = ["<select a song>", "Thrift Shop - Macklemore & Ryan Lewis", "Learn To Fly - Foo Fighters"]
# file_dropdown = st.selectbox("Or select an example MP3 file if you don't have your own.", options=song_list, )
class_labels = ['blues', 'classical', 'country',
                'disco', 'hiphop', 'metal', 'pop', 'reggae', 'rock']

# if file_dropdown != "<select a song>":
#     file = st.audio(f"song_examples/{file_dropdown}.mp3", "audio/mp3")
...
if file is not None:
    ...
    album = load_img(f'{filename} Spotify MP3 Album/Image_1.jpg')

    st.write(f"### {filename}")
    st.audio(file, "audio/mp3")
    st.image(album, use_column_width=True)
